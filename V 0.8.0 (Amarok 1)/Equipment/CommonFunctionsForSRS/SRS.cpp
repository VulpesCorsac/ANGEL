...
bool SRS::setConnection(QSerialPort* *this_serial, const QString &portName, const int &new_baudrate, const int &stopbit, const int &parity) const
{
...
QString SRS::autoDetect(const QString &portName, const int &new_baudrate, const int &stopbit, const int &parity) const
{
...
QString SRS::autoDetect(const QString &portName) const
{
    for (int stopbit = 1; stopbit <= 3; stopbit++)
        for (int parity = 0; parity <= 2; parity++)
            for (int baud = 0; baud < this->baudrateList.size(); baud++) {
                QString response = autoDetect(portName, this->baudrateList[baud].toInt(), stopbit, parity);
                if (!response.trimmed().isEmpty())
                    return response;
                QTest::qWait(20*this->defaultReadTimeout);
            }
    return "";
}
