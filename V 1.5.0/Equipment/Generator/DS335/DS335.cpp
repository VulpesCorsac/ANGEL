...
DS335::DS335(const QString &portName, const int &baudrate, const int &stopBit, const int &parity)
{
    this->serial = nullptr;
    this->srs = nullptr;
...
bool DS335::setConnection(const QString &portName, const int &baudrate, const int &stopBit, const int &parity)
{
...
bool DS335::isValidBaudrate(const int &baudrate) const
{
    return isValidString(this->baudrateList, QString::number(baudrate));
}

bool DS335::changeBaudrate(const int &baudrate)
{
...
void DS335::setWriteTimeout(const int &new_writeTimeout)
{
    return this->srs->setWriteTimeout(new_writeTimeout);
}

int DS335::getWriteTimeout() const
{
    return this->srs->getWriteTimeout();
}

void DS335::setReadTimeout(const int &new_readTimeout)
{
    return this->srs->setReadTimeout(new_readTimeout);
}

int DS335::getReadTimeout() const
{
    return this->srs->getReadTimeout();
}

void DS335::setReadWaitTimeout(const int &new_readWaitTimeout)
{
    return this->srs->setReadWaitTimeout(new_readWaitTimeout);
}

int DS335::getReadWaitTimeout() const
{
    return this->srs->getReadWaitTimeout();
}

void DS335::setAttemptsToConnect(const int &new_attemptsToConnect)
{
    return this->setAttemptsToConnect(new_attemptsToConnect);
}

int DS335::getAttemptsToConnect() const
{
...
bool DS335::isValidAmplitude(const double &amplitude, const QString &waveform, const QString &outputZ, const bool &VRMS) const
{
    return (getMinAmplitude(waveform, outputZ, VRMS) <= amplitude &&
            amplitude <= getMaxAmplitude(waveform, outputZ, VRMS));
}

bool DS335::setAmplitude(const double &amplitude, const bool &VRMS) const
{
    QString command = "AMPL " + QString::number(amplitude);
    if (VRMS)
        command += "VR";
    else
        command += "VP";
...
bool DS335::setOffset(const double &offset) const
{
    if (!isValidOffset(offset))
        return false;

    QString command = "OFFS " + QString::number(offset);
    return sendCommand(command);
}

double DS335::getOffset() const
{
...
bool DS335::setFrequency(const double &frequency) const
{
    QString command = "FREQ " + QString::number(frequency,'f',8);
    return sendCommand(command);
}
...
bool DS335::setFunction(const int &function) const
{
    if (!isValidNumber(this->functionList, function))
        return false;

    QString command = "FUNC " + QString::number(function);
    return sendCommand(command);
}
...
bool DS335::setInverse(const bool &inverse) const
{
...
bool DS335::setOutputImpedance(const int &outputImpedance) const
{
    ...
    QString command = "TERM " + QString::number(outputImpedance);
    return sendCommand(command);
}

bool DS335::setOutputImpedance(const QString &outputImpedance) const
{
